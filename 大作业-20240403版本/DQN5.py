...
def main():
    ...
    for e in range(episodes):
        with open("./data/testcase5.in", "r") as file:
        ...
        #     agent.model.load_state_dict(torch.load(f"./model/model5_agent_{agent_id}.pt"))
        ...
        while step < 15000:
            ...
            with open("./data/testcase51.out", "a") as file:
                # 更新战斗机状态
            ...
            if any(fighter.fuel == 0 for fighter in fighters):
                break
            # for agent in agents:
            #     agent.remember(state, action, reward, next_state, done)
            #     agent.learn(batch_size)
            ...
            if max_score < score:
                max_score = score
                for agent_id, agent in enumerate(agents):
                    torch.save(agent.model.state_dict(), f"./model/model5_agent_{agent_id}.pt")
