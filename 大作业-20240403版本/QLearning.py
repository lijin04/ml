...
def main():
    ...
    for e in range(episodes):
        with open("./data/testcase1.in", "r") as file:
        ...
        action_size = 4  # 修改动作空间大小为5
        agents = [DQNAgent(state_size, action_size, agent_id) for agent_id in range(len(fighters))]
        # 加载预训练的模型参数
        for agent_id, agent in enumerate(agents):
            agent.model.load_state_dict(torch.load(f"./model/model3_agent_{agent_id}.pt"))
        ...
        while step < 15000:
            ...
            with open("./data/testcase1.out", "a") as file:
                # 更新战斗机状态
                for fighter, action, agent in zip(fighters, actions, agents):
                    ...
                    state = next_state 
                    print("episodes", e," step ", step, "OK, ","score: ",score," fighter id: ",fighter.id," reward: ",reward," action: ",action," fuel: ",fighter.fuel)  # 表示该帧指令输出结束，并刷新输出                       
                file.write("OK\n")
            ...
            if step < minstep:
                minstep = step
                for agent_id, agent in enumerate(agents):
                    torch.save(agent.model.state_dict(), f"./model/model3_agent_{agent_id}.pt")
                    agent.update_target_model()
