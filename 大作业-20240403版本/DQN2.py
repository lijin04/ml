    ...
    def learn(self, batch_size):
        ...
        for state, action, reward, next_state, done in minibatch:
            ...
            if not done:
                ...
                # print(self.model(next_state_tensor).detach().numpy())
            ...
            # print(output)
            loss = criterion.forward(output, target_f)
            # print(loss)
            loss.backward()
...
def main():
    ...
    for e in range(episodes):
        with open("./data/testcase2.in", "r") as file:
        ...
        state_size = len(fighters) * 4 + len(blue_bases) * 4 + len(red_bases) * 6 + len(fighters) * len(blue_bases) + len(fighters) * len(red_bases)
        print(state_size)
        ...
        #     agent.model.load_state_dict(torch.load(f"./model/model2_agent_{agent_id}.pt"))
        ...
        while step < 15000:
            ...
            with open("./data/testcase21.out", "a") as file:
                # 更新战斗机状态
            ...
            if max_score < score:
                max_score = score
                for agent_id, agent in enumerate(agents):
                    torch.save(agent.model.state_dict(), f"./model/model2_agent_{agent_id}.pt")
